...
def load(filepath):
    ...
    out_dict = {"dims": None, "data": None, "acq_param": None, "workflow_type": None}
...
def check_filepaths(filepath):
    ...
    if filepath_temp.exists():
        if filepath_temp.suffix == ".DSC":
            ...
            raise ValueError(
                "Filepath must point to a .DSC or .DTA file, not : {filepath}"
            )
    else:
    ...
    if not DTA_filepath.exists():
        raise FileNotFoundError(
            f"DTA file does not exist at : {str(filepath_temp.parent)}"
        )
    if not DSC_filepath.exists():
        raise FileNotFoundError(
            f"DSC file does not exist at : {str(filepath_temp.parent)}"
        )
    ...
    with open(dsc_filepath, "r") as dsc_file:
        for line in dsc_file:
            ...
            if param_val_pair:
                ...
                if re.match(
                    r"^-?\d+\.?\d*$", value
                ):  # store numeric value as float or int
...
def get_DTA_datatype(dsc_parameter_dict):
    ...
    if "IKKF" in dsc_parameter_dict:
        ...
        warnings.warn(
            "IKKF keyword was not read from .DSC file, assuming data is real."
        )
    ...
    if "IRFMT" in dsc_parameter_dict:
        ...
        raise ValueError(
            "IRFMT keyword, which specifies the format of real values, could not be read from .DSC file."
        )
    ...
    if "BSEQ" in dsc_parameter_dict:
        ...
        raise ValueError(
            "BSEQ keyword, which specifies the byte order of data, could not be read from the .DSC file."
        )
...
def get_dim_arrays(dsc_parameter_dict, dta_filepath):
    ...
    axis_names = ["Z", "Y", "X"]
    axis_types = [
        dsc_parameter_dict[axis_name + "TYP"]
    ...
    for idx, ax_typ in enumerate(axis_types):
        if npts_tup[idx] > 1:
            if ax_typ == "IDX":
                ...
                gf_data = read_GF_file(
                    axis_names[idx], dsc_parameter_dict, dta_filepath
                )
...
def read_GF_file(ax_name, dsc_parameter_dict, dta_filepath):
    ...
    if gf_filepath.exists():
        if ax_name + "FMT" in dsc_parameter_dict:
            ...
            raise ValueError(
                ax_name
                + f"FMT keyword, which specifies the format of {ax_name} axis, could not be read from .DSC fie"
            )
...
class EprData:
    ...
    def __init__(self, out_dict):
        ...
        if self.acq_param["XNAM"] == "Field":
            ...
            self.g = (float(self.acq_param["MWFQ"]) / 1e9) / (13.996 * (x_g / 10000))
        else:
            self.g = None
    ...
    def baseline_correct(
        ...
        eprdata_proc = _baseline_correct(
            self,
            interactive,
            npts,
            method,
    ...
    def select_region(self, region):
        assert type(region) in [range, list], (
            "region keyword must be a range object or list."
        )
    ...
    def workflow(
        ...
        if self.pulse_program == "HYSCORE":
            hyscore_out_dict = EprWorkflow(
                eprdata=self,
                zf=zf,
                poly_order=poly_order,
                x_max=x_max,
                pick_eseem_points=pick_eseem_points,
                symmetrise=symmetrise,
                verbose=verbose,
            ).hyscore()
            return EprData(hyscore_out_dict)

        elif self.pulse_program in [
            "2P ESEEM",
            "3P ESEEM",
            "2P ESEEM vs. B0",
            "3P ESEEM vs. B0",
            ...
            raise ValueError(
                f"No supported workflows found for the pulse program : {self.pulse_program}"
            )
